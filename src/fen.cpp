	...
	for (i = 0;sq_90 < 90 && c != '\0'; ++i)
	{
		...
		else if ( (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		{
			...
			if (piece != PieceNone)
			{
				...
				board->piece[PIECE_TO_INDEX(piece) + board->number[piece]] = sq_256;
				board->square[sq_256] = PIECE_TO_INDEX(piece) + board->number[piece];
				...
				board->number[piece]++;
				board->piece_size[PIECE_TO_COLOUR(piece)]++;
...
bool board_to_fen( board_t *board, char fen [], int size )
{
	...
	for(i = 0; i < 90; i++)
	{
		...
		if(board->square[SQUARE_FROM_90(i)] != PieceNone)
		{
			...
			fen[index++] = IndexToChar[board->square[SQUARE_FROM_90(i)]];
		}
		else
			++n;
	}
