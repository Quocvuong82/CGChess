...
bool board_is_ok( board_t *board )
{
	...
	if( board->number[RedKing16] != 1 )
		return false;

	if( board->number[BlackKing16] != 1 )
		return false;
	...
	rking_sq = board->piece[IndexRedKing];
	bking_sq = board->piece[IndexBlackKing];
...
void board_init_list( board_t *board )
{
	...
	for (sq_90 = 0; sq_90 < 90; ++sq_90)
	{
		...
		if (piece != PieceNone)
		{
			if(INDEX_TO_PIECE16(piece) != IndexRedKing && INDEX_TO_PIECE16(piece) != IndexBlackKing)
