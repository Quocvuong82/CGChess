...
static void process_input()
    ...
    if( false ) { }
        ...
        {
        gen_default_ini_file("CGChess.cfg");
        }
    else if( string_equal(string, "random") )
        {
        gen_random_ini_file("CGChess.cfg");
        }	
    ...
    else if( string_start_with(string, "position ")  ||  string_start_with(string, "fen "))
        {
        if( !Searching && !Delay )
            {
        ...
        if( !Searching && !Delay )
            {
				my_timer_t usedtime[1];

				my_timer_reset( usedtime );
				my_timer_start( usedtime );

				send("perft %d " S64_FORMAT,atoi(string+5),perft(SearchInput->board,atoi(string+5)-1));				
               
				send("time: %f", my_timer_elapsed_real(usedtime));
...
static void parse_go( char string [] )
    ...
    if( COLOUR_IS_RED(SearchInput->board->turn) )
        {
        time = wtime;
        inc = winc;
        }
