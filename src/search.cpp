...
static bool do_null( board_t *board );
static bool do_ver( board_t *board );
static void pv_fill( const mv_t pv [], board_t *board );
static bool move_is_dangerous( int move, board_t *board );
static bool capture_is_dangerous( int move, board_t *board );
static bool simple_stalemate( board_t *board );
static bool passed_pawn_move( int move, board_t *board );
static int capture_value( int move, board_t *board );
static bool is_passed( const board_t *board, int to, int me );
// functions
		...
		if( depth > 1 || (depth == 1 && SearchCurrent[ThreadId]->multipv == SearchInput->multipv) )
		{
			for ( i = 0; i <= SearchInput->multipv; i++ )
			{
				if( save_multipv[i].mate == 0 )
				{
					...
					{
						send("info multipv %d depth %d seldepth %d score %d time %.0f nodes " S64_FORMAT " pv %s",
							i
							+1,
					...
					{
						send("info multipv %d depth %d seldepth %d score %d lowerbound time %.0f nodes "
							S64_FORMAT " pv %s",
							i
							+1,
							save_multipv[i].depth,
					...
					{
						send("info multipv %d depth %d seldepth %d score %d upperbound time %.0f nodes "
							S64_FORMAT " pv %s",
							i
							+1,
							save_multipv[i].depth,
	...
	int ThreadId )
{
	...
	ASSERT(value_is_ok(best_value[0]));

	list_sort(list);

	ASSERT(SearchBest[ThreadId]->move == LIST_MOVE(list, 0));
	ASSERT(SearchBest[ThreadId]->value == best_value[0]);
	...
	bool extended, int ThreadId )
{
	...
	{ // no legal move
		...
		else
		{
			...
			return VALUE_MATE(height);;//ValueDraw;
		}
	}
	...
	int trans_move, int *best_move, bool extended, int ThreadId )
{
	...
	*best_move = MoveNone;
	...
	{ // no legal move => stalemate
		ASSERT(board_is_stalemate(board));
		best_value =  VALUE_MATE(height + 1);;//ValueDraw;
	}
...
static int full_quiescence( board_t *board, int alpha, int beta, int depth, int height, mv_t pv [], int ThreadId )
{
	...
	if( !in_check )
	{
		...
		{
			return  VALUE_MATE(height + 1);//ValueDraw;
		}
	...
	bool extended, bool *cap_extended, int ThreadId )
{
	...
	*cap_extended = false;
	...
	int piece_nb = board->piece_size[Red] + board->piece_size[Black];

	if( in_pv && piece_nb <= 5 && capture != Empty && !INDEX_IS_PAWN(capture) )
		return depth;
...
static bool move_is_dangerous( int move, board_t *board )
{
	...
	int king;
	int me, opp;
	int king_me, king_opp;

	me = board->turn;
	opp = COLOUR_OPP(me);

    //king_me = KING_POS(board, me);
	king_opp= KING_POS(board, opp);
	...
	if( INDEX_IS_PAWN(piece) && (is_passed(board, MOVE_TO(move), board->turn))) 
		return true;

    if( INDEX_IS_CANNON(piece))
	{		
		if(EQUAL_FILE(to, king_opp) )
		{
            row = SQUARE_RANK(to);
			col = SQUARE_FILE(to);
		...
		else if( EQUAL_RANK(to, king_opp))
		{
            row = SQUARE_RANK(to);
			col = SQUARE_FILE(to);
			rowbit = board->rank[row];
...
static bool capture_is_dangerous( int move, board_t *board )
{
	...
	piece = MOVE_PIECE(move, board);
	 ...
	 //即将进九宫的兵
	if( INDEX_IS_PAWN(piece) && IN_CITY(MOVE_TO(move)))
	{
		return true;
	}
	...
	if( INDEX_IS_KING(capture) )
		return true;

	if( INDEX_IS_ROOK(capture))
		return true;

	if(INDEX_IS_ADVISOR(capture))
		return true;
...
static bool simple_stalemate( board_t *board )
{
	...
	list_t alist[1];
	gen_legal_moves(alist, board );
	if(LIST_SIZE(alist) > 0)
	{
       return false;
	}
...
int capture_value( int move, board_t *board )
{
    ...
    // 
	if( INDEX_IS_PAWN(capture) && is_passed(board, to, COLOUR_OPP(board->turn)) )
		value += 50;
	if( INDEX_IS_PAWN(piece) && is_passed(board, to, board->turn) )
		value += 50;
	
	if(INDEX_IS_CANNON(capture))
	{
       if(EQUAL_FILE(capture, king_me) || EQUAL_RANK(capture, king_me))
	   {
          value += 50;
	   }
	}
	if(INDEX_IS_CANNON(piece))
	{
       if(EQUAL_FILE(piece, king_opp) || EQUAL_RANK(piece, king_opp) )
	   {
          value += 50;
	   }
	}

	if(INDEX_IS_KNIGHT(capture))
	{
		if(to == C1 || to == G1 || to == C8 || to == G8)
            value += 50;
	}
	if(INDEX_IS_KNIGHT(piece))
	{
...
bool is_passed( const board_t *board, int to, int me )
{
	...
	if (me == Red )
    {
		if(rank <= Rank4)
		{
			//if(SQUARE_IS_OK(to - 16) && board->square[to-16] != BlackPawn)
			   return true;
		}
    }
	else
	{
		if(rank >= Rank5)
		{
			//if(SQUARE_IS_OK(to + 16) && board->square[to+16] != RedPawn)
			   return true;
		}
	}
