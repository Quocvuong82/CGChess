...
static void material_comp_info( material_info_t *info, const board_t *board )
{
	...
	rpawn = board->number[RedPawn16];
	rknight=board->number[RedKnight16];
	rcannon=board->number[RedCannon16];
	rbishop=board->number[RedBishop16];
	radvisor=board->number[RedAdvisor16];
	rrook  = board->number[RedRook16];

	bpawn = board->number[BlackPawn16];
	bknight=board->number[BlackKnight16];
	bcannon=board->number[BlackCannon16];
	bbishop=board->number[BlackBishop16];
	badvisor=board->number[BlackAdvisor16];
	brook  = board->number[BlackRook16];
